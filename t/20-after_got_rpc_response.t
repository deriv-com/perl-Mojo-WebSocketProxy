...
package t::FrontEnd {
    ...
    sub startup {
         ...
         $self->plugin(
             'web_socket_proxy' => {
                actions => [
                    ['success', { after_got_rpc_response => sub {
...
test_wsp {
    ...
    is JSON::MaybeXS->new->decode(Encode::decode_utf8($t->message->[1]))->{success}, 'success-reply';
    is JSON::MaybeXS->new->decode(Encode::decode_utf8($t->message->[1]))->{after_got_rpc_response}, 'ok';
} 't::FrontEnd';
